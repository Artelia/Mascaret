...
class CurveSelectorWidget(QWidget):
    ...
    def init_cb_graph(self, ini_graph=None):
        ...
        if self.cur_scen:
            if self.typ_graph in ["struct", "weirs"]:
                self.lst_graph = [{"type_res": self.typ_res, "id": 'gate_move', "name": 'Gate movement', "unit": 'm', "vars": ["ZSTR"]}]
    ...
    def init_cb_det(self, ini_det=None):
        ...
        if self.typ_graph in ["struct", "weirs"]:
            ...
            sql = "SELECT init_date FROM {0}.runs WHERE id = {1} ".format(self.mgis.mdb.SCHEMA, self.cur_scen)
    ...
    def get_lst_graph_opt(self):
        ...
        self.lst_graph = [{"type_res": 'opt', "id": 'Z', "name": 'Levels', "unit": '$m$',
                           "vars": ['ZREF', 'Z', 'ZMIN', 'ZMAX', 'CHAR']},
                          {"type_res": 'opt', "id": 'Q', "name": 'Flow rate', "unit": '$m^3/s$',
                           "vars": ['Q', 'QMIN', 'QMAJ', 'QMAX']}]

        common_var_exists = {'Q': False, 'QMIN': False, 'QMAJ': False, 'QMAX': False,
        ...
        if self.cur_scen:
            ...
            sql = "SELECT * FROM {0}.results_var WHERE id in {1}".format(self.mdb.SCHEMA, list_sql(list_vars))
        # else:
        ...
        if not rows:
            ...
            for rws in rows:
                if not rws[2] in common_var_exists.keys():
                    self.lst_graph.append({"type_res": rws[1], "id": rws[2], "name": rws[3], "unit": '', "vars": [rws[2]]})
                else:
    ...
    def get_lst_graph_bl(self):
        ...
        if self.typ_res in self.info_graph.keys():
            ...
            for rws in rows:
                self.lst_graph.append({"type_res": self.typ_res, "id": rws[2], "name": rws[3], "unit": '', "vars": [rws[2]]})
...
class SlideCurveSelectorWidget(CurveSelectorWidget):
    def __init__(self, mgis=None, row=None, typ_graph=None, typ_res=None, x_var=None, dict_run=None, cur_pknum=None, cur_branch=None):
        CurveSelectorWidget.__init__(self, mgis, "slide", row, typ_graph, typ_res, x_var, dict_run, cur_pknum, cur_branch)
...
class CompareCurveSelectorWidget(CurveSelectorWidget):
    def __init__(self, mgis=None, row=None, typ_graph=None, typ_res=None, x_var=None, dict_run=None, cur_pknum=None, cur_branch=None):
        CurveSelectorWidget.__init__(self, mgis, "compare", row, typ_graph, typ_res, x_var, dict_run, cur_pknum, cur_branch)
