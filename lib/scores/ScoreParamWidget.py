...
class ScoreParamWidget(QWidget):
    ...
    def init_gui(self):
        ...
        if len(self.lst_runs) > 0:
            ...
            for col, id_run in enumerate(self.lst_runs):
                ...
                # ************ add style widget ************
                for ctrl_ in ["per_start_t", "pt_start_t", "per_last_t", "pt_last_t", "ref_time"]:
                    self.widget_d[id_run][ctrl_].setCalendarPopup(True)
                    self.widget_d[id_run][ctrl_].setDisplayFormat("yyyy. MM.dd hh:mm:ss")

                    self.widget_d[id_run][ctrl_].setDateTimeRange(
                        QDateTime(1800, 1, 1, 00, 00, 00), QDateTime(9999, 1, 1, 00, 00, 00)
                    )
                ...
                if isinstance(date_init, datetime):
                    ...
                    date_init = datetime(tmps.year, tmps.month, tmps.day, 0, 0, 0)
                    final_date = datetime(tmps.year, tmps.month, tmps.day, 1, 0, 0)
                ...
                # ******** add control ************
                self.widget_d[id_run]["ref_time"].dateTimeChanged.connect(self.ch_date_ref)

                for ctrl_ in ["per_start_t", "pt_start_t"]:
                    self.widget_d[id_run][ctrl_].dateTimeChanged.connect(self.ch_date_limit)
                for ctrl_ in ["per_last_t", "pt_last_t"]:
                    self.widget_d[id_run][ctrl_].dateTimeChanged.connect(self.ch_date_limit)
                ...
                self.ch_persistence.stateChanged.connect(self.fct_chang_persistence)
    ...
    def cpt_score(self):
        ...
        if len(self.model.keys()) > 0:
            ...
            for id_run in self.no_obs:
                name_run = "{} - {}".format(dict_name[id_run]["run"], dict_name[id_run]["scenario"])
            ...
            for id_run in self.model.keys():
                ...
                if self.ch_ns_err.isChecked():
                    if not self.cpt_ns_err(id_run):
                        add_txt += "- Nash - Sutcliffe criterion " "(Run : {})\n".format(name_run)
                    else:
                ...
                if self.ch_persistence.isChecked():
                    ...
                    if (init_mod < last <= lst_mod) and (lst_mod > fst >= init_mod):
                        ...
                        if not test:
                            ...
                            if len(lst_test) > 0:
                                add_txt += (
                                    "- Persistance error (Run : {}), Time step"
                                    " no constant : \n".format(name_run)
                                )
                    ...
                    if "persistence" in self.res.keys():
                        ...
                        add_txt += (
                            "- Persistance error, No data in time rang"
                            " (Run : {})\n".format(name_run)
                        )
                ...
                if self.ch_pointe_err.isChecked():
                    ...
                    if (init_mod < last <= lst_mod) and (lst_mod > fst >= init_mod):
                        ...
                        add_txt += "- Tips error, No data in time rang" " (Run : {})\n".format(
                            name_run
                        )
        ...
        for pk in self.data[id_run].keys():
            for code in self.data[id_run][pk].keys():
                if self.cmpt_var[id_run][pk][code]["H"]:
                    ...
                    self.res["simple"][id_run][pk][code]["H"] = self.creat_dict_simple(
                ...
                if self.cmpt_var[id_run][pk][code]["Q"]:
                    self.add_res_dict("simple", id_run, pk, code)
                    self.res["simple"][id_run][pk][code]["Q"] = self.creat_dict_simple(
            ...
            for code in self.data[id_run][pk].keys():
                if self.cmpt_var[id_run][pk][code]["H"]:
                    ...
                    self.res["quantil"][id_run][pk][code]["H"] = {
                        "dist_err": dist_err(y_obs, y_pred, dist_step),
                        "dist_abs_err": dist_abs_err(y_obs, y_pred, dist_step),
                    }
    ...
    def cpt_persistence(self, id_run):
        ...
        for pk in self.data[id_run].keys():
            for code in self.data[id_run][pk].keys():
                if self.cmpt_var[id_run][pk][code]["H"]:
                    new_obs, new_obs_times = self.filter_time(
                        frst_time,
                        last_time,
                        self.data[id_run][pk][code]["h_obs"],
                        self.data[id_run][pk][code]["h_obs_time"],
                    )
                    new_pred, new_pred_times = self.filter_time(
                        frst_time,
                        last_time,
                        self.data[id_run][pk][code]["h_mod"],
                        self.data[id_run][pk][code]["h_obs_time"],
                    ...
                    self.res["persistence"][id_run][pk][code]["H"] = {
                        "per_err": persistence(new_obs, new_pred, new_obs_times, deltat),
                    }
            ...
            for code in self.data[id_run][pk].keys():
                if self.cmpt_var[id_run][pk][code]["H"]:
                    ...
                    new_pred, new_pred_times = self.filter_time(
                        frst_time,
                        last_time,
                        self.data[id_run][pk][code]["h_mod_ori"],
                        self.data[id_run][pk][code]["h_mod_ori_time"],
                    )
                    #
                    if len(new_obs) == 0:
                        continue
                    ...
                    self.res["tips_err"][id_run][pk][code]["H"] = {
                        "pts_err": err_point(new_obs, new_pred),
                        "pts_time_err": err_temps_point(
                            new_obs, new_pred, new_obs_times, new_pred_times, alphah
                        ),
                    }
                ...
                if self.cmpt_var[id_run][pk][code]["Q"]:
                    ...
                    new_pred, new_pred_times = self.filter_time(
                        frst_time,
                        last_time,
                        self.data[id_run][pk][code]["q_mod_ori"],
                        self.data[id_run][pk][code]["q_mod_ori_time"],
                    )
                    #
                    if len(new_obs) == 0:
                        continue
                    ...
                    self.res["tips_err"][id_run][pk][code]["Q"] = {
                        "pts_err": err_point(new_obs, new_pred),
                        "pts_time_err": err_temps_point(
                            new_obs, new_pred, new_obs_times, new_pred_times, alphaq
                        ),
                    }
                    self.type_res["tips_err"] = True
                    out = True
    ...
    def get_model(self, id_run, pk, code):
        ...
        if len(lst_varh) > 0:
            ...
            self.data[id_run][pk][code]["h_mod_ori_time"] = self.model[id_run]["times"]
        ...
        if len(lst_varq) > 0:
            ...
            self.data[id_run][pk][code]["q_mod_ori_time"] = self.model[id_run]["times"]
    ...
    def get_obs(self, id_run, pk, code):
        ...
        for gg in lst_var:
            ...
            if gg == "H" and len(tmp_dict["valeur"]) > 0:
                z = np.array(tmp_dict["valeur"]) + np.ones(len(tmp_dict["valeur"])) * code_zero
                obs_time = np.array(
    ...
    def info_model(self, id_run, init_time):
        ...
        final_time = init_time + timedelta(seconds=data_rg["time"][-1])

        info_var = self.mdb.select(
            "results_var",
            where="type_res = 'opt' " "AND var IN('Y', 'ZREF', 'Z', " "'Q', 'QMIN', 'QMAJ')",
            list_var=["id", "var"],
        )
        var_dict = {
            var: info_var["id"][i]
            for i, var in enumerate(info_var["var"])
            if info_var["id"][i] in data_rg["var"]
        }
        self.model[id_run] = {
            "init_time": init_time,
    ...
    def info_obs(self, id_run, init_time):
        ...
        for i, code in enumerate(rows["code"]):
            if code not in self.obs.keys():
                self.obs[code] = {"name": [], "abscissa": [], "zero": []}
    ...
    def get_parameter(self):
        ...
        for col, id_run in enumerate(self.lst_runs):
            for ctrl_ in self.widget_d[id_run].keys():
                self.param[id_run][ctrl_] = ctrl_get_value(self.widget_d[id_run][ctrl_])
    ...
    def check_lim(self):
        ...
        for col, id_run in enumerate(self.lst_runs):
            ref_time = self.widget_d[id_run]["ref_time"].dateTime().toPyDateTime()
