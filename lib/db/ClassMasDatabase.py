...
class ClassMasDatabase(object):
    ...
    def process_masobject(self, masobject, pg_method, schema=None, srid=None, **kwargs):
        """
    ...
    def register_object(self, obj):
        ...
        if key not in self.register:
            ...
            self.mgis.add_info("{0} already exists inside MasPlug registry.".format(key), dbg=True)
            #
    ...
    def add_vlayer(self, vlayer):
        ...
        try:
            style_file = os.path.join(
                self.mgis.masplugPath, "styles_qgis3", "{0}.qml".format(vlayer.name())
            )
            if self.group:
    ...
    def create_model(self, dossier):
        ...
        try:
            ...
            tables = [
                ...
                Maso.visu_branchs,
                Maso.branchs,
                Maso.visu_minor_river_bed,
                Maso.observations,
                Maso.parametres,
                Maso.runs,
                # Maso.laws,
                Maso.admin_tab,
                Maso.visu_flood_marks,
                # bassin
                Maso.basins,
                Maso.links,
                # qualite d'eau
            ...
            chkt.all_version(self.list_tables(self.SCHEMA), read_version(self.mgis.masplugPath))
    ...
    def schema_fct_sql(self):
        ...
        listefct = [
            ("pg_create_calcul_abscisse", self.SCHEMA),
    ...
    def add_fct_for_update_pk(self):
        ...
        lfct = [
            ...
            cl.pg_all_profil(self.SCHEMA),
            cl.pg_abscisse_point(self.SCHEMA),
            cl.pg_all_point(self.SCHEMA),
            cl.pg_abscisse_branch(self.SCHEMA),
            cl.pg_all_branch(self.SCHEMA),
        ]
        qry = ""
        for sql in lfct:
            qry += sql
...
"""
    ...
    def export_model(self, selection, file, plug_ver):
        ...
        err = self.export_schema(
            os.path.join(os.path.dirname(file), file_name + ".psql"), schem=js_dict["export_name"]
        )
        if not err:
