        ...
        if len(results) > 0:
            ...
            if couche == "profiles" and flag_profil_r:
                ...
                if results[0].mFeature["name"] in prof_a["name"]:
                    graph_res = GraphProfilResultDialog(
                        self.mgis, "hydro_profil", results[0].mFeature["abscissa"]
                    )
            ...
            if flag_hydro and (couche == "branchs" or couche == "visu_branchs"):
                ...
                if fbranch in branches["branch"]:
                    graph_hyd_pk = GraphResultDialog(self.mgis, "hydro_pk", fbranch)
                    graph_hyd_pk.show()
                else:
            ...
            if flag_hydro and couche in (
                ...
                if "extremities" == couche:
                    for var in ["name", "method", "active", "firstvalue", "type"]:
...
class GraphProfil(GraphCommon):
    ...
    def init_ui(self):
        ...
        (self.courbeMNT,) = self.axes.plot(
            [], [], color="grey", marker="o", markeredgewidth=0, zorder=90, label="MNT"
        )
        self.courbeTopo = []
        for i in range(12):
            (temp,) = self.axes.plot(
                [], [], color="green", marker="+", mew=2, zorder=95, label="Topo", picker=5
            )
            self.courbeTopo.append(temp)

        (self.courbedown,) = self.axes.plot(
            [], [], color="purple", marker="+", mew=2, zorder=95, label="downstream", picker=5
        )
        (self.courbeup,) = self.axes.plot(
            [], [], color="brown", marker="+", mew=2, zorder=95, label="upstream", picker=5
        )

        self.etiquetteTopo = []
        self.courbes = [self.courbeProfil, self.courbeMNT]

        self.lg_lit_mineur = self.axes.axvline(
            x=-9999999.0, color="red", ymin=0.0, ymax=1.0, ls=":", lw=2.5, zorder=85
        )
        self.ld_lit_mineur = self.axes.axvline(
            x=-9999999.0, color="red", ymin=0.0, ymax=1.0, ls=":", lw=2.5, zorder=85
        )
        self.lg_stock = self.axes.axvline(
            x=-9999999.0, color="green", ymin=0.0, ymax=1.0, ls=":", lw=2.5, zorder=85
        )
        self.ld_stock = self.axes.axvline(
            x=-9999999.0, color="green", ymin=0.0, ymax=1.0, ls=":", lw=2.5, zorder=85
        )
    ...
    def extrait_topo(self):
        ...
        if not requete:
            return
        table = zip(requete["gid"], requete["name"], requete["x"], requete["z"], requete["order_"])
        ...
        for nom in self.topo.keys():
            self.topo[nom]["x"] = self.mdb.projection(
                self.nom, self.topo[nom]["x"], self.topo[nom]["gid"]
            )
    ...
    def del_topo(self):
        ...
        name_topo, ok = QInputDialog.getItem(None, "Topo Choice", "Topo", liste)
        if ok:
    ...
    def import_topo(self):
        ...
        if fichiers:
            ...
            self.charger_bathy(fichiers, self.coucheProfils, self.nom)
        ...
        for fichier in liste:
            ...
            for f in couche_profil.getFeatures():
                if f["name"] == profil:
                    ...
                    tab = {"name": [], "profile": [], "order_": [], "x": [], "z": [], "geom": []}
    ...
    def import_image(self):
        fichier, _ = QFileDialog.getOpenFileName(
            None, "Sélection des fichiers", self.mgis.repProject, "Fichier (*.png *.jpg)"
        )
        ...
        try:
            ...
            self.image = self.axes.imshow(
                img, extent=[x0, x0 + lon, z0 - h, z0], zorder=1, aspect="auto"
            )
    ...
    def onpick(self, event):
        ...
        if (deplaceh and legline in self.courbeTopo and bouton == 1) or (
            ...
            if self.ordre == -9999:
                ...
                num, ok = QInputDialog.getInt(self, "Ordre", "Entrez l'ordre initial", ordre)
                if ok:
                    self.ordre = num
                else:
                    self.ordre = ordre
    ...
    def onselect_zone(self, xmin, xmax):
        ...
        if n > 0:
            ...
            self.mgis.add_info("The table has {0} elements, please add ".format(n), dbg=True)
        ...
        if self.flag and event.button == 3:
            if self.ordre == -9999:
                ...
                item, ok = QInputDialog.getItem(
                    self, "Curve", "Choice of Curve", self.topo.keys(), idx_topo
                )
                ...
                num, ok = QInputDialog.getInt(
                    self, "Order", "Input the initial order", self.order_topo
                )
                if not ok:
                    return
                ...
                if self.topoSelect not in self.topo.keys():
                    self.topo[self.topoSelect] = {"x": [], "z": [], "ordre": [], "gid": []}
    ...
    def display_planim(self):
        if self.ui.cb_planim.isChecked():
            ...
            self.axes.set_yticks(
                np.arange(min(ta["z"]), max(ta["z"] + [self.planim]), self.planim), minor=True
            )
    ...
    def maj_legende(self):
        ...
        self.leg = self.axes.legend(
            self.courbes, liste_noms, loc="upper right", fancybox=False, shadow=False
        )
    ...
    def del_line(self):
        ...
        if self.tableau.selectedIndexes():
            rows = [idx.row() for idx in self.tableau.selectedIndexes() if idx.row() > 0]
            rows = list(set(rows))
            ...
            for i, x in enumerate(ta["x"]):
                ...
                if i in rows:
                    if i == lenta - 1:
                        ...
                        newta["x"].append(self.fct1((ta["x"][i] + ta["x"][i + 1]) / 2.0))
                        newta["z"].append(self.fct1((ta["z"][i] + ta["z"][i + 1]) / 2.0))
            ta["x"] = newta["x"]
            ta["z"] = newta["z"]
    ...
    def filtre(self):
        ...
        if meth == "f_pente":
            ...
            newx, newz, err = filter_dist_perpendiculaire(
                self.tab["x"], self.tab["z"], seuil, fixe_x=fixe_x
            )
        else:
            newx, newz = [], []
        if err != "":
    ...
    def retrieve_both_bed(self):
        ...
        if x_left is not None and x_right is not None:
            ...
            status_right, mess_right = verif_right_bed(
                x_right, self.tab["rightstock"], self.tab["x"]
            )
    ...
    def retrieve_left_stock(self):
        ...
        if new_x is not None:
            status, mess = verif_left_stock(
                new_x, [self.tab["leftminbed"], self.tab["rightminbed"]], self.tab["x"]
            )
            if status == "e":
                QMessageBox.critical(self, "Error", mess, QMessageBox.Ok)
                return
    ...
    def del_amont_aval_old(self):
        ...
        if reply == QMessageBox.Yes:
            condition = "name='{0}' AND profile='{1}'".format("upstream", self.nom)
            self.mdb.delete("topo", condition)

            condition = "name='{0}' AND profile='{1}'".format("downstream", self.nom)
            self.mdb.delete("topo", condition)
        ...
        if 0 <= idam:
            if self.liste["branchnum"][id] == self.liste["branchnum"][idam]:
                if self.liste["x"][idam]:
                    xamont = [float(val) for val in self.liste["x"][idam].split()]
                    zamont = [float(val) for val in self.liste["z"][idam].split()]
        ...
        if idav < len(self.liste["branchnum"]):
            if self.liste["branchnum"][id] == self.liste["branchnum"][idav]:
                if self.liste["x"][idav]:
                    xaval = [float(val) for val in self.liste["x"][idav].split()]
                    zaval = [float(val) for val in self.liste["z"][idav].split()]
        ...
        for cas in [-1, 0, 1]:
            if 0 <= id + cas < len(self.liste["x"]):
                if self.liste["x"][id + cas]:
                    ...
                    minz, area, rz, lz, rmin, lmin = self.val_prof(id + cas)
                    self.ch_prof[dcas[cas]] = {
                        "point_bas": minz,
    ...
    def check_prof_interp(self):
        ...
        nofind = {"point_bas": None, "sect_plein_bord": None}
        for cas in [-1, 0, 1]:
            if cas == 0:
                if "interpolation" in self.topo.keys():
                    minz, area = self.val_inter_prof(-1)
                    self.ch_prof_inter[dcas[cas]] = {
                        "point_bas": minz,
                        "sect_plein_bord": area,
                    }
                else:
    ...
    def gui_interpol(self, id, idam, idav, err):
        ...
        if dlg.interpol_prof:
            ...
            condition = "name='{0}' AND profile='{1}'".format("interpolation", self.nom)
            self.mdb.delete("topo", condition)
...
class CopySelectedCellsAction(QAction):
    ...
    def copy_cells_to_clipboard(self):
        if len(self.table_widget.selectionModel().selectedIndexes()) > 0:
            lst_r = [idx.row() for idx in self.table_widget.selectionModel().selectedIndexes()]
            lst_c = [idx.column() for idx in self.table_widget.selectionModel().selectedIndexes()]
