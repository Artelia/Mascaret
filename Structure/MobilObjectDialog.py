...
class ClassMobilObjectDialog(QDialog):
    def __init__(self, mgis, typ_obj):
        ...
        # self.lst_obj.clicked.connect(self.select_list)
    ...
    def fill_lst_objects(self, def_id=None):
        ...
        sql = "SELECT gid, name, active_mob FROM {0}.{1} WHERE active='t' " \
              "ORDER BY name".format(self.mdb.SCHEMA, self.obj_table)
        rows = self.mdb.run_query(sql, fetch=True)

        for r, row in enumerate(rows):
            obj_id, obj_name, obj_act = row
    ...
    def cb_change_meth(self, text):
        ...
        sql = "UPDATE {0}.{1} SET method_mob = '{2}' WHERE name = '{3}'".format(
            self.mdb.SCHEMA, "ee", val, self.name_cur
        )
