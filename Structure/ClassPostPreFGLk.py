...
class ClassInfoParamFG_Lk(object):
    ...
    def fill_param_to_db(self, db):
        ...
        sql = ('SELECT {1} '
               ...
               'ORDER BY linknum;').format( db.SCHEMA, ", ".join(lst_var))
        print(sql)
        ...
        for row in rows:
            id_link = row[0]
            if not id_link in self.param_fg.keys():
        ...
        sql = ('SELECT {1} '
               'FROM {0}.links_mob_val '
               'WHERE id_links in ({2}) '
               'ORDER BY id_links;').format(db.SCHEMA, ", ".join(lst_var),
                                           ', '.join([str(v) for v in self.list_actif]))
        rows = db.run_query(sql, fetch=True)
        if rows is None:
            self.param_fg = {}
