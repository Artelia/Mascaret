...
class ClassBradley:
    ...
    #*****************************************
        ...
        for hn in list_hn:
            for q in list_q:
                ...
                ssoh=self.parent.coup_poly_v(poly_wet,[self.param_g['firstw'],self.param_g['TOTALW']],typ='LR').area
                q1 =ssoh *umoy
                ...
                dkp = np.interp(j, self.dico_abc['DKp_abac']['J'], self.dico_abc['DKp_abac'][str(self.param_pil['FORMPIL'])])
                # print('Dkp', dkp)
                coefs = np.interp(coefm, self.dico_abc['s_abac']['M'], self.dico_abc['s_abac'][str(self.param_pil['FORMPIL'])])
                # print('s', coefs)
                dkp=dkp*coefs
                # print('sDkp',dkp)
                ...
                list_e_interp =[]
                for ee in list_e:
                    list_m_interp.append(np.interp(coefm, self.dico_abc['dKe_abac']['M'], self.dico_abc['dKe_abac'][ee[0]]))
                ...
                if self.param_g['BIAIOUV'] == 0 :
                    ...
                    if self.param_g['BIAIOUV'] > 45:
                        dkx=np.interp(coefm, self.dico_abc[abac_dks]['M'], self.dico_abc[abac_dks]['phi>45'])
                    else:
                        list_ph = []
    ...
    def modif_xcas_str(self,fichier_cas):
        ...
        ouvrages= self.mdb.select('struct_abac', "active", "abscissa")
